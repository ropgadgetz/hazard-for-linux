...
if __name__ == "__main__":
    if os.path.basename(sys.argv[0]).endswith("exe"): #never will happen, unless you for some dumb fucking reason change the file extension to .exe
        search_for_updates()
        with open("/tmp/hazard_proxies", 'w'): pass
        if not os.path.exists("/tmp/hazard_theme"):
            setTheme('hazardous')
        clear()
        proxy_scrape()
        sleep(1.5)
